/* =========================================================================
 * Created on: <Sun Mar 29 13:45:09 +01 2026>
 * Time-stamp: <Sun Mar 29 16:38:32 +01 2026 by owner>
 * Author    : owner
 * Desc      : ~/coding/c_prog/csapp/netp/tiny/tiny09.c -
 *             tiny.c - A simple, iterative HTTP/1.0 Web server that uses
 *             the GET method to serve static and dynamic content.
 *
 * Problem 11.9: Modify Tiny so that when it serves static content,
 * it copies the requested file to the connected descriptor using malloc,
 * rio_readn, and rio_writen, instead of mmap and rio_writen.
 * Explanation:
 * [[file:~/coding/c_prog/csapp/netp/README.org::#problem-11-9]]
 * ========================================================================= */
#include "csapp.h"
...
void serve_static(int fd, char *filename, int filesize) {
  ...
  /* NOTE: Phase 2: Body delivery vio malloc() */
  srcp = Malloc(filesize); /* Allocate user memory */
  /* Get data from disk → to Kernel page cache (Memory Resident Permanent) */
  srcfd = Open(filename, O_RDONLY, 0);
  /* First copy from Kernel page cache → to user memory */
  Rio_readn(srcfd, srcp, filesize);
  Close(srcfd); /* Close right after reading. */
  /* Second copy from user memory → to Kernel socket buffer */
