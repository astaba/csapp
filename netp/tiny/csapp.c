...
void Bind(int sockfd, struct sockaddr *my_addr, int addrlen) {
  ...
  /* FIXME: BUG: fd leaks: release fd before exit. */
}
...
ssize_t rio_writen(int fd, void *usrbuf, size_t n) {
  ...
  while (nleft > 0) {
    if ((nwritten = write(fd, bufp, nleft)) <= 0) {
      ...
      } else if (errno == EPIPE) { /* INFO: Problem 11.13 */
        fprintf(stderr, "Client closed connection prematurely (Broken pipe)\n");
        return -1; /* Return error condition to the caller */
      } else
...
void Rio_writen(int fd, void *usrbuf, size_t n) {
  if (rio_writen(fd, usrbuf, n) != n) {
    if (errno == EPIPE)
      ...
      /* HACK: Log error but don't crash the server. */
      fprintf(stderr, "Rio_writen() failed: %s\n", strerror(errno));
    }
  }
}
