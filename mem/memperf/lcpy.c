...
double getcpufrequency() {
  ...
  /* if used on a MP system, and hw counters are not (almost) synchronized, bad
     things may happen! on some unix systems it would be possible to use
     bind_to_cpu, but linux doesn't support this */
  for (i = 0; i < 3; i++) {
    ...
    if ((modenr == modes) || (modes == 9)) {
      ...
      for (currit = 1; currit <= nrofrep; currit++) {
        ...
        /*
           ...
           %d\n",par_cid,p[par_cid].par_cid); printf("Address
           %d\n",&p[par_cid]);
                             */
