...
int main(int argc, char *argv[argc + 1]) {
  ...
  while (cumul_offset < file_size) {
    ...
    // backaligned_offset = cumul_offset - (cumul_offset % page_size)
    // That is: rounding the cumul_offset to the nearest page_size boundary.
    // From the express assumption that page_size is a power of 2, we can take
    // avantage of the better performance of binary operations to replicate the
    // same logic as follows:
